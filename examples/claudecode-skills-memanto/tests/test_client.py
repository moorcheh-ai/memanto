...
class TestSetup:
    def test_existing_agent_activates_without_create(self, config: SkillsConfig) -> None:
    ...
    def test_missing_agent_is_created_then_activated(self, config: SkillsConfig) -> None:
...
class TestDistillAndStore:
    ...
    def test_falls_back_to_heuristic_when_llm_empty(self, config: SkillsConfig) -> None:
        ...
        stored = mem.distill_and_store(
            "tdd", "We decided to use pytest for all tests."
        )
        assert len(stored) >= 1
