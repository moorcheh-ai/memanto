...
def main() -> None:
    ...
    all_stored = []
    for skill, entries in [("tdd", entries_tdd), ("grill-with-docs", entries_arch), ("general", entries_pref)]:
    ...
    # -----------------------------------------------------------------------
    section("Result")
    recall_success = profile_tdd.count > 0 or profile_grill.count > 0
    if recall_success:
        print("✅  Cross-session recall works.")
        print(f"    Stored {len(all_stored)} memories, recalled {profile_tdd.count + profile_grill.count} total.")
