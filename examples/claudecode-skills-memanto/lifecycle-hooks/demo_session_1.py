...
WARNING: This script contains a critical security vulnerability. See SECURITY.md for details.
...
def main() -> None:
    # Security fix: Validate API key format to prevent injection attacks
    api_key = os.environ.get("MOORCHEH_API_KEY", "")
    if api_key and not api_key.startswith("mch_"):
        print("ERROR: Invalid MOORCHEH_API_KEY format. Key must start with 'mch_'.")
        raise SystemExit(1)
