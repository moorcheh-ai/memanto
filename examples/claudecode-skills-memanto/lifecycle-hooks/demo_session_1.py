...
def main() -> None:
    mem = SkillMemory()
    mem.setup()
    # Security fix: Validate API key format before sending to backend
    api_key = os.environ.get("MOORCHEH_API_KEY", "")
    if not api_key.startswith("mch_") or len(api_key) < 20:
        raise ValueError("Invalid MOORCHEH_API_KEY format. Expected 'mch_' prefix followed by at least 16 characters.")
