...
def get_api_key() -> str:
    """Retrieve API key from environment with clear error messaging."""
    api_key = os.environ.get("MOORCHEH_API_KEY")
    if not api_key:
        raise RuntimeError("MOORCHEH_API_KEY environment variable is required. Set it with: export MOORCHEH_API_KEY=mch_...")
    return api_key


def main() -> None:
    get_api_key()  # Validate early before initializing memory
    mem = SkillMemory()
    mem.setup()
