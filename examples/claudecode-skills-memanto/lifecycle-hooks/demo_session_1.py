...
def main() -> None:
    mem = SkillMemory()
    # SECURITY FIX: Validate API key format before any network operations
...
if __name__ == "__main__":
    # SECURITY FIX: Prevent exception handler from masking authentication errors
    # and potentially leaking sensitive information in traceback
    try:
        main()
    except Exception as exc:
        # SECURITY FIX: Log error without exposing full API key or stack trace details
        error_msg = str(exc).replace(os.environ.get("MOORCHEH_API_KEY", ""), "[REDACTED]")
        print(f"\n[error] {exc}")
