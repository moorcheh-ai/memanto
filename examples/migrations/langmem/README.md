...
The recorded import succeeded before an export-path error; validation resumed without importing duplicates. `original-cli-import.txt` records the successful import; `cli-import.txt` and `live-report.json` record the resumed validation with import explicitly skipped. Source JSON is 1,086 bytes, local OKF is 4,856 bytes, and the cloud re-export including session context is 10,578 bytes. No savings are claimed.

The committed `live-report.json` is the original workflow output, and its `target_recall` equals `target-recall.json`. The observations about expected facts above come from inspecting those recorded hits. For publication, trailing whitespace was removed from CLI logs, metrics, and session context; memory documents were unchanged. The committed `target-okf/` therefore totals 10,548 bytes, while the report retains the 10,578-byte measurement from the original export. This normalization does not alter the report or claim a new cloud run.
...
runner does not activate or delete agents and refuses an existing output directory. Import adds memories to the explicitly selected agent. Exports are first written to a unique directory under `~/.memanto/exports` as the shipped CLI requires, then copied to the run directory. Those export files remain available locally. The final run directory is published only after exact snapshot validation succeeds; failures remove the temporary run directory so its output path can be retried. This cleanup does not undo cloud writes: after an import or transport failure, inspect the target before retrying, and do not blindly repeat an import that may have succeeded. Use `--resume-after-import` only after confirming an earlier import succeeded; the report marks import as skipped and does not invent its timing.
