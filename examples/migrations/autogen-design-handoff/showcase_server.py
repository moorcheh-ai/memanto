...
STATE = {
    ...
    "done": False,
    "exit_code": None,
    "log": "Ready to run the actual AutoGen → Memanto pipeline.",
}
LOCK = threading.Lock()
    ...
    def do_GET(self):
        ...
        if self.path == "/":
            ...
            with LOCK:
                state = dict(STATE)
            for name, key in [
                ("source-operations.json", "source"),
                ("migration-summary.json", "summary"),
                ("recall-after.json", "recall"),
                ("ranked-top3.json", "ranked"),
            ]:
                path = RUN / name
                if path.exists():
                    try:
                        ...
                        pass
            candidates = list(
                (RUN / "exported-okf" / "memories" / "decision").glob("*finish.md")
            )
            if candidates:
