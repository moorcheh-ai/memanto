...
def load_memory_database(
    ...
    with _connect_read_only(memory_db) as connection:
    ...
    try:
        ...
        safe_source_memories = [
            SourceMemory(
                **{
                    **asdict(memory),
                    "raw_memory": redactor.redact(memory.raw_memory),
                    "rollout_summary": redactor.redact(memory.rollout_summary),
                    "cwd": redactor.redact(memory.cwd) or None,
                    "rollout_path": redactor.redact(memory.rollout_path) or None,
                    "git_branch": redactor.redact(memory.git_branch) or None,
                    "thread_title": redactor.redact(memory.thread_title) or None,
                }
            )
        ...
        mapped = map_source_memories(
            safe_source_memories, redactor=redactor, stats=stats
        )
        if not mapped:
            raise ValueError(
