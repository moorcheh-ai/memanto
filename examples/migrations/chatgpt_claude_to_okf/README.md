...
# 2) Convert to a portable OKF bundle (use a dedicated real-data output dir —
#    `okf_bundle/` is the committed sample and is regenerated by `./run.sh`)
python convert.py chatgpt ./path/to/export --out okf_bundle_real
python convert.py claude  ./path/to/export --out okf_bundle_real

# 3) Validate against the real Memanto CLI (no API key needed for dry-run)
memanto migrate okf ./okf_bundle_real --dry-run
...
| Reusability & cleanliness (20) | Single-command `run.sh`, `requirements.txt` (pinned), 11 pytest tests, clean layout ready to merge into `examples/migrations/` | `tests/` |
