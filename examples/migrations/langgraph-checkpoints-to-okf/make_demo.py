...
Requires: pillow, imageio, imageio-ffmpeg (pip install).
"""
...
FONT_B = ImageFont.truetype(FONT_PATH, 21)
LINE_H = 26
PAD = 34
MAX_CHARS = 100
FPS = 30
...
def main() -> None:
    ...
    for _ in range(FPS * 2):
        writer.append_data(_to_bytes(card))
    ...
    for i, ln in enumerate(transcript):
        ...
        if i % step == 0 or i == len(transcript) - 1:
            writer.append_data(_to_bytes(frame(shown)))
    # hold on final (4s)
    final = _to_bytes(frame(shown))
