...
def remember_preference(content: str) -> str:
    ...
    """
    memory = _get_client().remember(content=content, memory_type="preference", tags=["preference","user"])
    mid = memory.get('id')
    if not mid:
...
def remember_decision(content: str) -> str:
    ...
    memory = _get_client().remember(content=content, memory_type="decision", tags=["decision"])
    mid = memory.get('id')
    if not mid:
...
def answer_from_memory(question: str) -> str:
    ...
    return f"🧠 Memory-grounded answer: {answer}" if answer else "No memory-grounded answer generated."
