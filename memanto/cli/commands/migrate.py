...
def _build_chat_export_report_markdown(
    *,
    ...
    if exported_at:
        lines.append(f"_{provider_name} export: {exported_at}_")
    
    lines.extend([
        "",
        "## Your Footprint (measured)",
        "",
        "| Metric | Value |",
        "| --- | --- |",
    ...
    if narrative:
        lines.extend([
            ...
            narrative,
            "",
            f"_Narrative generated by {llm_model} via {llm_method}_",
        ])
    
    return "\n".join(lines)


_PROVIDER_BUNDLES.update({
    "chatgpt": {
        "label": "ChatGPT",
