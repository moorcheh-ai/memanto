...
class TestMEMANTOAPI:
    ...
    async def test_batch_remember_per_document_upload_status_counts(
        ...
        assert response.status_code == 200
        body = response.json()
        # Successful, failed, and unconfirmed counts must all be present
        assert "successful" in body
        assert "failed" in body
        assert "unconfirmed" in body, (
            "API must return unconfirmed count even when zero"
        )
        assert body["successful"] + body["failed"] + body["unconfirmed"] == 2
    ...
    @pytest.mark.asyncio
    async def test_batch_remember_aggregate_only_marks_unconfirmed(
        self, client, auth_headers, mock_moorcheh
    ):
        """Aggregate success without per-document results must mark all unconfirmed."""
        await client.post(
            "/api/v2/agents",
            headers=auth_headers,
            json={"agent_id": self.TEST_AGENT_ID},
        ...
        # Aggregate success response without per-document results/details
        mock_moorcheh.documents.upload.return_value = {"status": "success"}
        ...
        response = await client.post(
            ...
            json={
                "memories": [
                    {"content": "Unconfirmed A", "type": "fact"},
                    {"content": "Unconfirmed B", "type": "fact"},
                ]
            },
        )
        assert response.status_code == 200
        body = response.json()
        # No per-document confirmation — all must be unconfirmed, not success
        assert body["successful"] == 0, (
            f"Aggregate-only success must not report documents as successful"
        )
        assert body["unconfirmed"] == 2, (
            f"Expected 2 unconfirmed, got {body.get('unconfirmed')}"
        )
        assert "results" in body
        for r in body["results"]:
            assert r.get("status") == "unconfirmed", (
                f"Expected unconfirmed status, got {r.get('status')}; "
                f"error={r.get('error')}"
            )
