...
---
## ??????????????
**??????????** `memanto memory export --okf` ?[Open Knowledge Format](https://docs.memanto.ai/integrations/okf)?????????????diff?commit?grep ???????? Markdown ?????????????????????????????
...
---
## ?????
**`pip install` ???** ????????embedding ???????reranker??????????????????????????????????

**??????????????** `memanto connect claude-code`?Cursor?Codex?Windsurf?Cline?Continue?Goose?Copilot ????????????

**???????????** ???????????????????????????????????`remember` ???????????????????

**???????????** `instruction`?`fact`?`decision`?`goal`?`preference`?`relationship` ?? 13 ??????????????
...
|---|---|---|
| ?????? | `memanto status` | ???????????????????????????????????? |
| ???? REST API + Web UI | `memanto serve`, `memanto ui` | REST API ??????????? UI ???? |
...
```
?????:[CLI User Guide](https://docs.memanto.ai/cli)
</details>
...
**??????:**
<p align="center">
  <img alt="?????????????" src="https://github.com/moorcheh-ai/memanto/raw/main/assets/On-prem-architecture-diagram.png" width="900">
</p>
</details>
...
| | |
|---|---|
| [**Recall ????????**](https://youtu.be/zoKP4b_rUhY) — 6:20 | [**?????????**](https://www.youtube.com/watch?v=vEtOaoweIG4) |
...
?? recall ???????? LongMemEval 89.8%?LoCoMo 87.1% ?????????<!-- TODO: reader model?judge model?subset ??????????????????? --> ???????????? [huggingface.co/moorcheh](https://huggingface.co/moorcheh) ??????????

??????:??????????????????????????????reader model?judge model?judge prompt??????????????????????????????????????????????????????????????????????????????????? recall ??????????????????????6 ??????????
